...
int main(int argc, char* argv[]){
	...
	double rho_max, rho_min, omega, tolerance, time;
	int p, q, m;
...
//Check proper entry arguments
	if(argc<9){
		cout << "Bad usage. Enter also 'outfilename rho_max rho_min max_matrix_size electrons_2_1 interaction_1_0 omega' on same line." << endl;
		exit(1);
	}
	else{
		outfilename = argv[1];
	...
	ofile << "#_n";
	for(i=0;i<5;i++){
		ofile << setw(15) << "eigenv_" << i;
	}
	ofile << endl;
	ofiletime << "#_n" << setw(15) << "sim rot" << setw(15) << "time" << endl;
	for(p=1;p<m;p++){
		for(q=2;q<11;q+=2){
			rows = q*pow(10.0,p);
			columns = rows;
		...
		//Write output file
			...
			ofiletime << rows << setw(15) << count << setw(15) << time << endl;
			cout << endl;
