	...
	for(j=0;j<bodies;j++){
		...
		for(k=0;k<bodies;k++){
			if(j!=k){
				...
				relcor = (threeovercsq/rsq)*(relangmomentum[j][k*4+3]*relangmomentum[j][k*4+3])*(masssqj/masssqk);
...
void massivesystem::initialize(massivesystem& sol, double**& output, double*& mass, int steps){
	...
	for(j=0;j<bodies;j++){
		...
		output[0][j*8+1] = sol.system[j].position[0];
		output[0][j*8+2] = sol.system[j].position[1];
		output[0][j*8+3] = sol.system[j].position[2];
		output[0][j*8+4] = sol.system[j].position[3];
		output[0][j*8+5] = sol.system[j].velocity[0];
		output[0][j*8+6] = sol.system[j].velocity[1];
		output[0][j*8+7] = sol.system[j].velocity[2];
		output[0][j*8+8] = sol.system[j].velocity[3];
	}
	for(i=1;i<steps+1;i++){
		for(j=0;j<bodies;j++){
			output[i][j*8+1] = 0;
...
void massivesystem::verlet(massivesystem& sol, double**& output, int steps, double tmax){
...
void massivesystem::RK4(massivesystem& sol, double**& output, int steps, double tmax){
	int i, j, k, m, n, bodies;
	bodies = sol.massivebody_count;
	double h, halfh, halfhsq, rcubed, fourpisq;
	double* k1, *k2, *k3, *k4;
